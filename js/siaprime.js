...
const siacoinsToHastings = siacoins =>
  new BigNumber(siacoins).times(hastingsPerSiacoin)
const hastingsToSiacoins = hastings =>
  new BigNumber(hastings).dividedBy(hastingsPerSiacoin)
...
const launch = (path, settings) => {
  ...
  const flags = Object.keys(mergedSettings)
    .filter(filterFlags)
    .map(mapFlags)
  ...
  const siadOutput = (() => {
    if (typeof mergedSettings['siaprime-directory'] !== 'undefined') {
      return fs.createWriteStream(
        Path.join(mergedSettings['siaprime-directory'], 'spd-output.log')
      )
    }
