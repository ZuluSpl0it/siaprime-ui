...
describe('wallet send prompt component', () => {
	...
	it('renders send amount with the correct currency', () => {
		expect(sendpromptComponentSC.find('.sendamount h3').first().text()).to.contain('Send Amount (SCP)')
		expect(sendpromptComponentSF.find('.sendamount h3').first().text()).to.contain('Send Amount (SPF)')
	})
