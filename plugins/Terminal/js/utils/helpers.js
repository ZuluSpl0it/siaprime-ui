...
export const checkSiaPath = () =>
  new Promise((resolve, reject) => {
    fs.stat(SiaAPI.config.attr('spc').path, err => {
      if (!err) {
        if (
          Path.basename(SiaAPI.config.attr('spc').path).indexOf('spc') !== -1
        ) {
          resolve()
        } else {
...
export const initPlugin = () =>
  checkSiaPath().catch(() => {
    ...
    SiaAPI.config.attr('spc', {
      path: Path.resolve(
        Path.dirname(SiaAPI.config.attr('spd').path),
        process.platform === 'win32' ? './spc.exe' : './spc'
      )
    })
    checkSiaPath().catch(() => {
      // config.path doesn't exist. Prompt the user for siac's location
      if (!SiaAPI.config.attr('spc')) {
        SiaAPI.config.attr('spc', { path: '' })
      }
      SiaAPI.showError({
        title: 'Spc not found',
      ...
      const siacPath = SiaAPI.openFile({
        ...
        properties: ['openFile'],
        defaultPath: Path.join('..', SiaAPI.config.attr('spc').path || './'),
        filters: [{ name: 'spc', extensions: ['*'] }]
      })
      if (siacPath) {
        if (Path.basename(siacPath[0]).indexOf('spc') === -1) {
          SiaAPI.showError({
            title: 'Invalid Binary Name',
            content:
              'The spc binary must be called spc. Restart the plugin to choose a valid binary.'
          })
        } else {
          SiaAPI.config.attr('spc', { path: siacPath[0] })
        }
      } else {
        SiaAPI.showError({
          title: 'Spc not found',
          content:
            'This plugin will be unusable until a proper spc binary is found.'
        })
      }
    })
    SiaAPI.config.save()
...
export const spawnCommand = function (commandStr, actions, newid) {
  ...
  if (commandString.startsWith('spc')) {
    commandString = commandString.slice(4).trim()
  } else if (commandString.startsWith('./spc')) {
  ...
  const siac = child_process.spawn(SiaAPI.config.attr('spc').path, args, {
...
export const httpCommand = function (commandStr, actions, newid) {
  ...
  // Add address flag to siac.
  let siaAddr = url.parse('http://localhost:4280')
