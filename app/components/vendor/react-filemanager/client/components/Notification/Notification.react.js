...
export default class Notification extends Component {
  ...
  render() {
    ...
    const toggleElement =
      minimizable && (progressText || children) ? (
        <div
          tabIndex="0"
          className="oc-fm--notification__header-icon"
          onClick={this.handleToggleClick}
        >
          <SVG svg={minimized ? maximizeIcon : minimizeIcon} style={{ fill: '#f5f5f5' }} />
        </div>
      ) : null

    const closeElement = closable ? (
      <div tabIndex="0" className="oc-fm--notification__header-icon" onClick={onHide}>
        <SVG svg={closeIcon} style={{ fill: '#f5f5f5' }} />
      </div>
    ) : null

    const progressMessageElement = progressText ? (
      <div className="oc-fm--notification__progress">
        <div className="oc-fm--notification__progress-text">{progressText}</div>
